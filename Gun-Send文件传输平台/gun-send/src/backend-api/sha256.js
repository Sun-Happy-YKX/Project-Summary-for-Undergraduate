...
function sha256Ssigma0 (x) {
  return (rotateRight(7, x) ^ rotateRight(18, x) ^ (x >>> 3))
}
function sha256Ssigma1 (x) {
  return (rotateRight(17, x) ^ rotateRight(19, x) ^ (x >>> 10))
}
function sha256Expand (W, j) {
  return (W[j & 0x0f] += sha256Ssigma1(W[(j + 14) & 0x0f]) + W[(j + 9) & 0x0f] +
  sha256Ssigma0(W[(j + 1) & 0x0f]))
}
...
var sha256HexDigits = '0123456789abcdef'
...
/* Main function: returns a hex string representing the SHA256 value of the
  given data */
const hashcode = function sha256Digest (data) {
  sha256Init()
